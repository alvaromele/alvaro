...
class RolesModel extends Mysql
{
	...
	public function selecRoles()
	{
		...
		//EXTRAE ROLES
		$sql = "SELECT * FROM rol WHERE status != 2" . $whereAdmin;
		$request = $this->select_all($sql);
		return $request;
	}

	public function selecRol(int $idrol)
	{
		//BUSCAR ROLE
		$this->intIdrol = $idrol;
	...
	public function actualizarRol(int $idrol, string $rol, string $descripcion, int $status)
	{
		$this->intIdrol = $idrol;
		$this->strRol = $rol;
		$this->strDescripcion = $descripcion;
	...
	public function borrarRol(int $idrol)
	{
		$this->intIdrol = $idrol;
